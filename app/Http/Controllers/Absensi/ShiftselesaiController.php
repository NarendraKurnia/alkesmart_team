...
class ShiftselesaiController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // --- LOGIKA FILTER NAMA LOGIN ---
        // Mengambil nama user yang sedang login (memastikan data yang muncul hanya milik user tersebut)
        $userName = \Illuminate\Support\Facades\Auth::user()->name;
        
        $all = $all->filter(function ($item) use ($userName) {
            return $item->nama === $userName;
        });
        // --------------------------------

        // Filter pencarian berdasarkan keywords tetap berjalan pada data yang sudah disaring
        ...
        $shift_selesai = new LengthAwarePaginator(
            ...
            $perPage,
            $page,
            ['path' => url('absensi/selesai'), 'query' => $request->query()]
        );

        return view('absensi/layout/wrapper', [
            'title'         => 'Riwayat Selesai Kunjungan Saya',
            'shift_selesai' => $shift_selesai,
            'content'       => 'absensi/selesai/index'
        ]);
    }
